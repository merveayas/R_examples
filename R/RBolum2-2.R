#######  R ile Istatistiksel Veri Analizine Giris Bolum 2-Kisim 2  ###########
...
## Bu haftaki kursla ilgili sorularinizi Google groups ta sorabilirsiniz.
...
library(ez)
repanova2<-ezANOVA(data = Indometh,dv=conc,wid=Subject,within=time,type=1)  # dv=dependent variable ezANOVA komutu ile repeated measures ANOVA
print(repanova2)
...
library(gplots)
plotmeans(Indometh$conc~Indometh$time,xlab="Time",ylab="Concentration", main="Means Plot\nwith 95% CI",n.label=FALSE)   # CI grafigi
...
cor(Sepal.Length[Species !="versicolor"],Petal.Length[Species!="versicolor"])   
plot(Sepal.Length[Species !="versicolor"],Petal.Length[Species!="versicolor"])   
with(subset(iris,Species!="versicolor"),cor(Sepal.Length,Petal.Length))
with(subset(iris,Species!="versicolor"),plot(Sepal.Length,Petal.Length))
...
library(psych)
corr.test(iris[,1:4])
partial.r(iris[,1:4],x=c(1,2),y=c(3))            #Sepal.Length ile Sepal.Width arasinda Pedal.Length etkisi alindiktan sonraki Kismi korelasyon katsayisi
