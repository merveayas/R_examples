...
## Link=> https://www.datacamp.com/community/tutorials/make-histogram-ggplot2
library(ggplot2)

gp<-ggplot(mtcars,aes(x=mpg))+geom_histogram(binwidth = 5, col="black", fill="white",linetype="dashed")
plot(gp)
theme_set(theme_bw())
#theme_set(theme_grey())
# Density cizmek isterseniz asagidaki kodu kullanabilirsiniz
# gp<-ggplot(mtcars, aes(x=mpg)) +
#  geom_histogram(aes(y=..density..), colour="black", fill="white")+
#  geom_density(alpha=.2, fill="#FF6666") 
#plot(gp)

mtcars$vs<-factor(mtcars$vs)
gp<-ggplot(mtcars,aes(x=mpg,color=vs,fill=vs))+geom_histogram(binwidth = 5, alpha=0.2)
plot(gp)

h<-hist(data,breaks=20,ylim=c(0,40),xlim=c(0,140))
...
## ggplot2
ggplot(chickwts,aes(x=feed,y=weight,fill=feed))+geom_boxplot()+scale_fill_brewer(palette="Dark2") + theme_classic()
ggplot(chickwts,aes(x=feed,y=weight,fill=gender))+geom_boxplot()
ggplot(chickwts,aes(x=gender,y=weight,fill=feed))+geom_boxplot()
#######  Scatterplot   ################
...
gg<-ggplot(mammals,aes(x=brain,y=body))+geom_point(size=3)+geom_smooth(method = "lm",se = FALSE,col="firebrick")
plot(gg)
gg<-gg+scale_x_continuous(trans='log10')+scale_y_continuous(trans='log10') 
plot(gg)
library(car)
